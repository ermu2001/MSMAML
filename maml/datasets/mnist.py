...
class MNISTMAMLSplit():
    def __init__(self, root, train=True, num_train_classes=100,
        ...
        if self._train:
            self._images = self._dataset['data']['train'].reshape([-1, 1, 28, 28])
            self._labels = self._dataset['label']['train']
        else:
            self._images = self._dataset['data']['test'].reshape([-1, 1, 28, 28])
...
class MNISTMetaDataset(object):
    def __init__(self, name='MNIST1d', root='data', 
                 ...
                 num_train_classes=10, num_workers=0, device='cpu'):
        self.name = name
        self._root = root
    ...
    def _get_mnist_data_loader(self):
        ...
        dset = MNISTMAMLSplit(self._root, transform=img_transform,
